...
/* ---------------------------------------------------------------------------------------------
	ENQUEUE PB CUSTOM STYLES AND SCRIPTS
	--------------------------------------------------------------------------------------------- */
function pb_custom_styles() {

	wp_enqueue_style( 'bootstrap4','https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css', false, null, 'all' );
	...
	wp_enqueue_script( 'isotope-settings', get_stylesheet_directory_uri() . '/assets/js/isotopes.js', array( 'jquery' ),'',true );

	wp_enqueue_style( 'pb-style', get_stylesheet_directory_uri() . '/pb-style.css', false, null, 'all' );
	// wp_enqueue_script( 'back-to-top', get_stylesheet_directory_uri() . '/assets/js/btt.js', array( 'jquery' ));
}
add_action( 'wp_enqueue_scripts', 'pb_custom_styles' );

/* ---------------------------------------------------------------------------------------------
	 LOAD CUSTOM PB POST TYPES
	 --------------------------------------------------------------------------------------------- */

$pbtheme_includes = array(
	'/cpt_agencies.php',								// Load agencies custom post type
	'/cpt_freelancers.php',							// Load freelancers custom post type
  '/cpt_featuredcreative.php',				// Load freelancers custom post type
  '/cpt_faq.php',											// Load freelancers custom post type
	'/cpt_internships.php',							// Load freelancers custom post type
	'/cpt_careers.php',									// Load freelancers custom post type


);

foreach ( $pbtheme_includes as $file ) {
	$filepath = locate_template( 'CPTs' . $file );
	if ( ! $filepath ) {
...
/* ---------------------------------------------------------------------------------------------
	 LOAD CUSTOM PB TAXONOMIES
	 --------------------------------------------------------------------------------------------- */

$pbtheme_includes = array(
	'/tax_size.php',									// Load agencies custom taxonomy: Size
  '/tax_keypeople.php',             // Load agencies custom taxonomy: Key People
  '/tax_clients.php',								// Load agencies custom taxonomy: Notable Clients
	'/tax_city.php',									// Load agencies custom taxonomy: city
	'/tax_capabilities.php',					// Load agencies custom taxonomy: Capabilities
	'/tax_industry.php',							// Load agencies custom taxonomy: Industry


);

foreach ( $pbtheme_includes as $file ) {
	$filepath = locate_template( 'tax' . $file );
	if ( ! $filepath ) {
		trigger_error( sprintf( 'Error locating /inc%s for inclusion', $file ), E_USER_ERROR );
	}
	require_once $filepath;
...
fa_custom_setup_kit('https://kit.fontawesome.com/87040b45a5.js');
...
function my_acf_init() {
    acf_update_setting('google_api_key', 'AIzaSyACJY-a02CL9sfjwFstvkfVooxyrEdEyeA');
}
...
fa_custom_setup_cdn_webfont(
  ...
  'sha384-REHJTs1r2ErKBuJB0fCK99gCYsVjwxHrSU0N7I1zl9vZbggVJXRMsv/sLlOAGb4M'
);
/**
...
/* ---------------------------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------------------------- */
// // redirect image attachment pages somewhere else. Avoids 'thin' file_get_contents
function pb_redirect_attachment_page() {
	if ( is_attachment() ) {
		global $post;
...
add_action( 'template_redirect', 'pb_redirect_attachment_page' );
